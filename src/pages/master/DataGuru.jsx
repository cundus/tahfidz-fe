...
import { Badge, Button, Flex, Icon, Input, Link, Select, Td, Tr, useDisclosure, useToast } from "@chakra-ui/react";
...
const DataGuru = () => {
  ...
  return (
    <>
      ...
      <Flex marginTop={10} justifyContent="space-between" alignItems="center">
        <Flex gap={3} alignItems="stretch">
          ...
          <ButtonCustom
            ...
            title="Reset"
            height="38px"
            onClick={() => (setQuery(""), setOption(""), handleGetAllGuru())}
          />
        </Flex>
      ...
      <TableCustom
        isLoading={loading}
        thead={["#", "No Induk Guru", "Nama Lengkap", "Jenis kelamin", "Status", "Aksi"]}
        tbody={
              ...
              <Tr key={idx}>
                ...
                <Td>
                  <Flex gap={3} color="#0D6EFD">
                    <Link href={`/master-data/data-guru/detail-data-guru/${data.id}`} color="#0D6EFD">
                      Detail
                    </Link>
                    <Link
                      mx="5px"
